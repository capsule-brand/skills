...
export function rowToCatalog(row) {
  return {
    ...
    // Resolve the stored icon id to inline SVG (same shape as skills.json seeds).
    // Unknown/empty → '' so the card falls back to the mono initials chip.
    icon: (row.icon && SKILL_ICONS[row.icon]) || '',
